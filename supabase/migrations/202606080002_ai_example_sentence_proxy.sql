-- Add example-sentence generation to server-side AI metering.
--
-- The task prefers local inference, but cloud fallback uses Grasp provider
-- secrets through ai-proxy, so it must be represented in the daily quota table.
