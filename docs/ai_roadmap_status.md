...
- [x] **`ai_usage_events`**：沿用既有 `AiAnalyticsService`（`ai_events`，記 taskType/provider/result/latency/failure_reason）+ **estimated input/output tokens**（`in_tokens`/`out_tokens`,只在 >0 時存）。新 `AiTokenEstimator`（純函式,CJK ~1 token/字、英數 ~4 字/token,適合中文重的學習內容）。`getUsageSummary({hours})` 彙總 calls/successes/tokens 並 per-provider 拆分,供成本遙測/用量 UI。已接:smartDistractors(Groq)、photoImport、conversationTurn(原本完全沒記到 ledger,現補上)。⚠️ speakingScore 尚未帶 token（scorer 未接 analytics）。新增 7 測試。
