...
### 決策日誌（隨手記，學習歷程素材）
- 檔案：`portfolio/決策日誌.md`（不進 git）。
- **每當做出一個值得寫進學習歷程的關鍵決策**（方向選擇、取捨、推翻先前假設、面對回饋的處理、架構或產品判斷），就主動 append 一筆——通常在 commit 後。瑣碎實作細節不必記。
- 每筆固定格式：決策 / 為什麼(思考) / 影響 / 人機協作觀察 / 學到的事 / 佐證 commit。
- 學習歷程成品檔：`portfolio/學習歷程_拾憶Grasp_*.md`。
